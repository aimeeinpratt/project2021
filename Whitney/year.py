...
out_filename = "uniqueyear-wh.csv"
out_filename2 = "uniqueartist-wh.csv"
...
with open(inp_filename, 'r', newline='') as in_WH:
    ...
    with open(out_filename, 'w', newline='') as out_WH:
        ...
        for row in reader:
            ...
            if artistcol not in Uniqueartist:
                Uniqueartist.append(artistcol)

            if final_date not in Uniqueyear:
                Uniqueyear.append(final_date)
                   

        for clean_up_year in Uniqueyear:
            writer.writerow([clean_up_year,Museum])
